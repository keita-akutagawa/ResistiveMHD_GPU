...
struct CalculateFluxFunctor {
    ...
    Flux operator()(
        ...
        >& tupleForFlux) const {
            ...
            float SL = hLLDParameter.SL;
            float S1L = hLLDParameter.S1L;
            float SM = hLLDParameter.SM;
            float SR = hLLDParameter.SR;
            float S1R = hLLDParameter.S1R;

            auto calculateFluxComponent = [&](
                float outerLeftFlux, float middleLeftFlux, float innerLeftFlux,
                float outerRightFlux, float middleRightFlux, float innerRightFlux
            ) {
                return outerLeftFlux   * (SL > 0.0f)
                     + middleLeftFlux  * ((SL <= 0.0f) && (0.0f < S1L))
                     + innerLeftFlux   * ((S1L <= 0.0f) && (0.0f < SM))
                     + outerRightFlux  * (SR <= 0.0f)
                     + middleRightFlux * ((S1R <= 0.0f) && (0.0f < SR))
                     + innerRightFlux  * ((SM <= 0.0f) && (0.0f < S1R));
            };
...
struct calculateHLLDParameterFunctor {
    ...
    HLLDParameter operator()(const BasicParameter& dQLeft,  const BasicParameter& dQRight) const {
        float rhoL, uL, vL, wL, bXL, bYL, bZL, eL, pL, pTL;
        float rhoR, uR, vR, wR, bXR, bYR, bZR, eR, pR, pTR;
        float csL, caL, vaL, cfL; 
        float csR, caR, vaR, cfR;
        float SL, SR, SM;
        float pT1, pT1L, pT1R;
        float rho1L, u1L, v1L, w1L, bX1L, bY1L, bZ1L, e1L;
        float rho1R, u1R, v1R, w1R, bX1R, bY1R, bZ1R, e1R;
        float S1L, S1R;
        float rho2L, rho2R, u2, v2, w2, bY2, bZ2, e2L, e2R, pT2L, pT2R;
        Sign sign;
        ...
        bZL  = dQLeft.bZ;
        pL   = dQLeft.p;
        eL   = pL / (device_gamma_mhd - 1.0f)
             + 0.5f * rhoL * (uL * uL + vL * vL + wL * wL)
             + 0.5f * (bXL * bXL + bYL * bYL + bZL * bZL); 
        pTL  = pL + 0.5f * (bXL * bXL + bYL * bYL + bZL * bZL);
...
struct setFluxFunctor {
    ...
    thrust::tuple<Flux, Flux, Flux, Flux, Flux, Flux> operator()(
        ...
        float rhoL, uL, vL, wL, bXL, bYL, bZL, eL, pTL;
        float rhoR, uR, vR, wR, bXR, bYR, bZR, eR, pTR;
        float rho1L, u1L, v1L, w1L, bY1L, bZ1L, e1L, pT1L;
        float rho1R, u1R, v1R, w1R, bY1R, bZ1R, e1R, pT1R;
        float rho2L, rho2R, u2, v2, w2, bY2, bZ2, e2L, e2R, pT2L, pT2R;
