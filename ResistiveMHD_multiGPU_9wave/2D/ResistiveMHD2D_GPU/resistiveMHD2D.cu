...
#include "const.hpp"
#include "resistiveMHD2D.hpp"


ResistiveMHD2D::ResistiveMHD2D(MPIInfo& mPIInfo)
    : mPIInfo(mPIInfo), 
...
)
{
    ...
    if ((0 < i) && (i < localSizeX) && (0 < j) && (j < localSizeY)) {
        ...
        UBar[index].psi  = U[index].psi 
                         - device_dt / device_dx * (fluxF[index].f8 - fluxF[index - localSizeY].f8)
                         - device_dt / device_dy * (fluxG[index].f8 - fluxG[index - 1].f8)
                         - device_dt * pow(device_ch / device_cp, 2) * U[index].psi;
    }
}
...
)
{
    ...
    if ((0 < i) && (i < localSizeX) && (0 < j) && (j < localSizeY)) {
        ...
        U[index].psi  = 0.5 * (U[index].psi + UBar[index].psi
                      - device_dt / device_dx * (fluxF[index].f8 - fluxF[index - localSizeY].f8)
                      - device_dt / device_dy * (fluxG[index].f8 - fluxG[index - 1].f8))
                      - device_dt / 2.0 * pow(device_ch / device_cp, 2) * 0.5 * (U[index].psi + UBar[index].psi);
    }
}
...
void ResistiveMHD2D::oneStepRK2_periodicXWallY()
{
    dim3 threadsPerBlock(16, 16);
    dim3 blocksPerGrid((mPIInfo.localSizeX + threadsPerBlock.x - 1) / threadsPerBlock.x,
                       (mPIInfo.localSizeY + threadsPerBlock.y - 1) / threadsPerBlock.y);
...
void ResistiveMHD2D::oneStepRK2_periodicXSymmetricY()
{
