...
std::string directoryname = "results_diffusion";
std::string filenameWithoutStep = "diffusion";
std::ofstream logfile("results_diffusion/log_diffusion.txt");
...
__global__ void initializeU_kernel(ConservationParameter* U) 
{
    ...
    if (i < device_nx && j < device_ny) {
        ...
        bY = 0.0f;
        bYHalf = 0.0f;
        bZ = device_b0 * exp(-(pow(x - xCenter, 2) + pow(y - yCenter, 2)) / pow(5.0f, 2));
        p = device_p0;
