...
)
{
    ...
    if (i < device_nx - 1 && j < device_ny - 1) {
        float eZF1, eZF2, eZG1, eZG2, eZ;

        eZG1 = fluxG[j + i * device_ny].f4;
        eZG2 = fluxG[j + (i + 1) * device_ny].f4;
        eZF1 = -1.0f * fluxF[j + i * device_ny].f5;
        eZF2 = -1.0f * fluxF[j + 1 + i * device_ny].f5;
        eZ = 0.25f * (eZG1 + eZG2 + eZF1 + eZF2);
...
__global__ void CT_kernel(
    const float* bXOld, const float* bYOld, 
    const float* eZVector, 
